...
export class Orientation {
    constructor(
        ...
        public b0: number, public b1: number, public b2: number, public b3: number, public start_angle: number
    ) { }
}
...
export class Layout {
    ...
    public static flat: Orientation = new Orientation(
        3.0 / 2.0, 0.0, Math.sqrt(3.0) / 2.0, Math.sqrt(3.0), 2.0 / 3.0, 0.0,
    ...
    public static polygonCornersAbs(layout: Layout, h: Hex): Point[] {
        var corners: Point[] = [];
...
export class HexMapFactory {
    public static hexagonMap(radius: number) {
        var tiles: Hex[] = [];
        for (var q: number = -radius; q <= radius; q++) {
            var r1: number = Math.max(-radius, -q - radius);
            var r2: number = Math.min(radius, -q + radius);
            for (var r: number = r1; r <= r2; r++) {
                tiles.push(new Hex(q, r, -q - r));
            }
        }
        return tiles;
    }
